                ...
                for (let j = 0; j < this.headers.length; j++) {
                    ...
                    if (this.headers[j]) {
                        if (this.headers[j].length > 0) {
                            if (this.headers[j][0] === '"')
                                this.headers[j] = this.headers[j].substring(1, this.headers[j].length - 1);
                            if (this.headers[j][this.headers[j].length - 1] === '"')
                                this.headers[j] = this.headers[j].substring(this.headers[j].length - 2, 1);
                        }
        ...
        return (
            <div>
                <label for="file-upload" className="custom-file-upload">
                    <FontAwesomeIcon icon={faUpload} size="lg" style={{ marginRight: '10px' }} />
                    Upload File
                </label>
                <input
                    id="file-upload"
