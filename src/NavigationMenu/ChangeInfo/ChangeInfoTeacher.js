...
function ChangeInfoTeacher() {
    ...
    const handleSubmit = (e) => {
        if (validator.isEmail(email) && phoneNumberValidator(phone) && !stringContainsNumber(name)) {
            axios
                .post(`/api/users/${username}/update`, {
                    email: email,
                    name: name,
                    phoneNumber: phone,
              ...
              /> */}
                ...
                {successMessage &&
                    (successMessage === 'Cập nhật thông tin thành công' ? (
                        <SuccessMessage style={{ color: 'blue' }}>{successMessage}</SuccessMessage>
                    ) : (
                        <SuccessMessage style={{ color: 'red' }}>{successMessage}</SuccessMessage>
                    ))}
...
const Red = styled.span`
    ...
    margin-left: 0px;
`;
const Wrapper = styled.div`background-color: #f4f5fa;`;
