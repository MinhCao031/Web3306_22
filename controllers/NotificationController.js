...
module.exports.getAllNotifications = async function (req, res) {
  ...
  let foundClass = undefined;
  if (foundUser.role == 'Teacher') {
    foundClass = await Class.findOne({ teacherId: foundUser.username }).catch(
      (err) => console.log(err)
    );
  } else {
    foundClass = await Class.findOne({ studentIds: foundUser.username });
  ...
  if (foundUser && foundClass) {
    ...
    foundNotifications = foundNotifications.filter(
      (notification) => notification.ownerId != foundUser.username
    );
